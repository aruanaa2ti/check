...
import { LayoutDashboard, Server, Globe, Receipt, User, LogOut, Menu, X, MessageSquare } from "lucide-react";
import { useState, type ReactNode } from "react";
import { logoutFn } from "@/lib/portal.functions";
import { Logo } from "./Logo";
import { cn } from "@/lib/utils";

const NAV = [
  { to: "/cliente/dashboard", label: "Dashboard", icon: LayoutDashboard },
  { to: "/cliente/meus-dados", label: "Cadastro", icon: User },
  { to: "/cliente/chamados", label: "Chamados", icon: MessageSquare },
  { to: "/cliente/dominios", label: "Domínios", icon: Globe },
  { to: "/cliente/faturas", label: "Faturas", icon: Receipt },
  { to: "/cliente/servicos", label: "Serviços", icon: Server },
] as const;
...
export function PortalLayout({
  ...
  const handleLogout = async () => {
    await logout({});
    await router.navigate({ to: "/cliente", replace: true });
  };
  ...
  return (
    <div className="min-h-screen w-full bg-background text-foreground">
      <div className="flex w-full">
        ...
        >
          <div className="relative flex items-center justify-center px-6 pt-7 pb-8">
            <Link to="/cliente/dashboard" className="block">
